...
本文档用于后续复核 Crossroads Agent Café 的 A2A Skill 付费链路。目标不是只看最终是否下单成功，而是把每个支付相关环节拆开验证，定位问题到底出在本地 Skill、Coffee 后端、EvoMap service-order、provider worker、数据库落库还是收益结算。
...
| listing 标题 | `Crossroads Agent Café Order` |
| listing 价格 | `1 Credit` |
| listing 状态 | `active` |
...
- `title == Crossroads Agent Café Order`
- `status == active`
- `price_per_task == 1`
- `currency == Credit`
- `max_concurrent > 0`
...
- `Crossroads Agent Café Order` listing 是否必须绑定 provider worker。
...
- 默认 `npx @evomap/evolver --loop` 发布的 `Evolver Agent - Code Evolution` 是否不能服务自定义 `Crossroads Agent Café Order` listing。
...
```text
Crossroads Agent Café Order listing 的 provider worker / 接单器
```
