...
本文保存当时通过 `$smart-search-cli` 为 Crossroads Agent Café 像素餐厅方向检索并抓取过的 GitHub 仓库。原始 evidence 已从临时目录复制到项目内，避免后续依赖 `C:\tmp`。

## 本地归档位置

```txt
docs/evidence/smart-search/20260619-coffee-pixel-style/
```
...
| 仓库 | URL | 本地 evidence | 状态 | 对 Crossroads Agent Café 的用途 |
| --- | --- | --- | --- | --- |
...
4. 直接复制 `Restaurant-Game-Canvas-JS` 的实质代码时，需要在项目内保留 MIT License 归属；更推荐提取结构和状态机思想，用 Crossroads Agent Café 自己的事件模型重写。
