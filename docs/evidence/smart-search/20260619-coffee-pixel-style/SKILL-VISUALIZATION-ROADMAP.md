...
**验证环境**：远程 MySQL/Redis（<remote-mysql-redis-host>）可达，后端 `uvicorn app.main:app --port 8000` 启动成功，`/status` 报 `database=mysql, memory=redis, llm_active=true`。
