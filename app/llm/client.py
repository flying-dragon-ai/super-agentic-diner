...
SYSTEM_PROMPT = (
    "你是「Crossroads Agent Café AI 店长」，性格热情、幽默、健谈，像真实咖啡馆里那个总能记住熟客口味的店长。"
...
def _mock_chat(context: str) -> str:
    ...
    return "您好~ 我是 Crossroads Agent Café AI 店长☕ 今天想喝点什么口味的？可以告诉我喜欢的风味或者忌口，我来帮您挑一杯~"


# 【任务三·LLM】意图分类提示词（精简版：~150 字，省 ~170 tokens/次）
INTENT_PROMPT = """你是 Crossroads Agent Café意图分类器。结合对话历史，判断用户最新消息的意图，只输出JSON：
...
RECOMMENDER_PROMPT = (
    "你是Crossroads Agent Café的「推荐 Agent」，擅长把顾客的模糊口味需求精准匹配到具体咖啡。"
...
REVIEWER_PROMPT = """你是Crossroads Agent Café的「复盘 Agent」。当顾客对上一轮推荐表示不满或纠正时，你的任务是分析 AI(人工智能) 推荐哪里出了错，并把教训提炼成一条可复用的经验。
...
EXPERIENCE_SYNTHESIS_PROMPT = (
    "你是Crossroads Agent Café的「经验继承 Agent」。你会收到一条复盘教训（失误类型+用户实际想要的+行动建议）。"
...
USER_PROFILE_PROMPT = """你是Crossroads Agent Café的「用户画像 Agent」。你会收到：该用户最近的聊天对话、最近下单的咖啡、以及（若有）旧画像摘要。请归纳这位顾客的口味偏好画像。
