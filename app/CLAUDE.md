...
- **生命周期**：
  - `startup` → `start_colyseus_server()`：以子进程拉起 `colyseus-server/`（优先 `node dist/Server.js`，回退 `npx tsx`）。
  - `shutdown` → `stop_colyseus_server()`：发 `CTRL_BREAK_EVENT`（Windows）/ `terminate` 终止子进程。
- **静态托管**：
  - `/static` → `app/static/`（2D 对话页 `index.html`）
  - `/3d/assets`、`/3d/office-assets` → `app/static/3d/`（Vite 构建产物）
  - `/` → 2D 对话页；`/3d`、`/3d/{path}` → 3D SPA（fallback 到 index.html）

## 对外接口

| 方法 | 路径 | 鉴权 | 说明 |
|------|------|------|------|
| POST | `/chat` | 无（匿名 user_id） | 对话点单主入口；返回回复 + 可选 order_id |
...
## 数据模型（`app/db/models.py`，8 张表）

| 表 | 说明 | 关键字段 |
|----|------|---------|
| `user` | 匿名顾客（带本地余额） | user_id, balance, taste_preference |
...
| `skill_order_ledger` | A2A 点单账本（幂等 + 支付凭证） | request_id(unique), amount_credits, payment_status, evomap_order_id, payment_proof_json |
| `visualization_event` | 可视化事件流（持久化） | event_type, payload_json, correlation_id, agent_id |

**CHECK 约束**：order.source_type / status / payment_status、agent/consumer.status、ledger.payment_status 都用域常量（`app/domain_constants.py`）做数据库级校验。
...
```
process_skill_order:
  幂等检查(request_id) → _resume_existing_order
  _resolve_items(直接名/价格/LLM/RAG/后缀匹配)
  免费额度判断(free_sequence <= SKILL_FREE_ORDER_LIMIT)
    ├─ 免费 → _complete_order(PAYMENT_STATUS_FREE)
    └─ 需付费:
        ├─ 无 node_secret → 抛 SkillPaymentRequired(402, 含 service_order_request)
        └─ 有 node_secret → place_service_order(EvoMap Hub) → _complete_order(PAYMENT_STATUS_PAID)
           失败 → ledger.payment_status=failed/needs_reconcile
```
...
| 文件 | 说明 |
|------|------|
| `main.py` | FastAPI 入口，所有路由 + `/chat` 决策树 |
| `config.py` | pydantic-settings 配置 |
| `domain_constants.py` | 订单/支付/身份状态枚举（数据库 CHECK 约束来源） |
| `colyseus_bridge.py` | Colyseus 子进程生命周期管理 |
| `db/database.py` | engine + SessionLocal + Base |
| `db/models.py` | 8 张 SQLAlchemy 表 |
| `db/seed.py` | 种子数据 |
| `services/chat_service.py` | RAG 聊天主流程 + 价格匹配 |
| `services/order_service.py` | 事务安全扣款下单（with_for_update） |
| `services/skill_order_service.py` | A2A Skill 点单全流程 |
| `services/evomap_payment_service.py` | EvoMap 积分 service-order 支付客户端 |
