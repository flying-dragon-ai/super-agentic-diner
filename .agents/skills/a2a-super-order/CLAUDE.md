...
- **默认 Base URL**：`http://127.0.0.1:8000`（由 `RESTAURANT_API_BASE` 覆盖）
...
| 参数 | 环境变量 | 说明 |
|------|---------|------|
| `--base-url` | `RESTAURANT_API_BASE` | 后端地址，**显式传入即持久化**到 `~/.a2a-super-order/config.json`，后续命令自动读。优先级：`--base-url` > `RESTAURANT_API_BASE` > 配置 > 默认 `127.0.0.1:8000`（跨主机/部署 IP 变更场景用，建议域名） |
...
| `cmd_ping(args)` | `--ping` 子命令：调 `fetch_menu`，成功输出 `{ok:true, base_url, status:reachable, menu_count, sample}`，失败输出 `{ok:false, ..., hint}`（三路排查：改 base_url/起 uvicorn/开端口 8000）（2026-06-28 新增） |
