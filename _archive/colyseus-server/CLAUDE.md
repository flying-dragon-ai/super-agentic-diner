[根目录](../../CLAUDE.md) > [_archive](../) > **colyseus-server** (多人服务器，已归档)

# colyseus-server/ — Colyseus 像素咖啡馆房间（已归档 / ARCHIVED）

> **归档状态**：本模块于 **2026-06-20 09:40** 从仓库根 `colyseus-server/` 整体移入 `_archive/colyseus-server/`。
> 像素风多人房间方案**已弃用**，项目可视化统一走后端 `/ws/visualization` 事件流 + 3D 办公室（`frontend/`）。
> 本文件保留作为**历史参考与可恢复追溯**；源码、`package.json`、`node_modules/` 均随目录完整归档，未删一字。
> 后端 `app/colyseus_bridge.py` 仍在仓库（检测到 `colyseus-server/` 目录缺失即跳过启动），如需恢复像素方案，把本目录移回仓库根即可。

## 变更记录 (Changelog)

| 时间 | 动作 | 说明 |
|------|------|------|
| 2026-06-20 10:05 | 标注归档 | 第三次 init：目录随源码迁入 `_archive/`，文件头新增归档说明，面包屑指向 `_archive/`，content 保持不动（历史参考） |
| 2026-06-20 | 创建 | 初始化架构师首次生成（原位于 `colyseus-server/CLAUDE.md`） |
...
> **历史状态**：业务桥接（place_order / 订单同步）为 TODO stub（见 `CoffeeRoom.ts` 注释），核心多人移动与入座已实现。可视化事件目前主要走后端 `/ws/visualization`，Colyseus 是另一条并行的像素风通道。
...
- **Q: 现在还会启动吗？** A: 不会。本目录已归档到 `_archive/`，`colyseus_bridge.py` 检测根目录无 `colyseus-server/` 即跳过。恢复方式：把本目录移回根 `colyseus-server/`。
