#!/usr/bin/env python3
"""
Coffee AI Boss — 服务器部署压缩包打包器。

排除开发期文件（.venv / .git / node_modules / 各 AI 工具配置 / 日志 / 设计文档），
生成可直接 `docker compose -f deploy/docker-compose.1panel.yml up -d --build` 的精简包。

用法：
    python scripts/pack-release.py [输出zip路径]
默认输出：<项目根>/coffee-ai-boss-release.zip

压缩包结构：顶层目录 coffee-ai-boss/，解压后即项目根（含 .env、Dockerfile、deploy/ 等）。
"""
...
OUT = Path(sys.argv[1]).resolve() if len(sys.argv) > 1 else ROOT / "coffee-ai-boss-release.zip"
ARC_PREFIX = "coffee-ai-boss"
...
EXCLUDE_FILES = {
    ...
    "coffee-ai-boss-release.zip",          # 避免自包含
