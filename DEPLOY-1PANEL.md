...
```
              ...
              └──────────┬───────────┘
                         ...
                         ▼
              ┌──────────────────────┐
              │  coffee-ai-boss 容器   │  127.0.0.1:8000
              │  (uvicorn, 单 worker) │
              └───────┬────────┬─────┘
                      │        │
          MySQL ──────┘        └───── Redis
...
> 你本地开发已连的就是这台 MySQL（`47.93.176.175`），库/用户大概率已存在；此步仅核对 `coffee` 用户允许从 docker 网段（`172.17.0.0/16` 或 `%`）连接。
...
```bash
sudo mkdir -p /opt/coffee-ai-boss && sudo chown $USER:$USER /opt/coffee-ai-boss
git clone <你的仓库地址> /opt/coffee-ai-boss
cd /opt/coffee-ai-boss
```

**方式 B（1Panel 文件管理）**：1Panel「主机」→「文件」→ 进 `/opt/`，上传打包好的代码压缩包并解压到 `/opt/coffee-ai-boss/`。
...
在 `/opt/coffee-ai-boss` 执行（1Panel 终端）：
```bash
...
docker ps | grep coffee-ai-boss                                # 确认运行
...
| 操作 | 命令 / 入口 |
|---|---|
| 查日志 | 1Panel「容器」→ coffee-ai-boss → 日志；或 `docker logs -f coffee-ai-boss` |
| 重启 | 1Panel「容器」→ 重启；或 `docker restart coffee-ai-boss` |
| 改了 `.env` | `cd /opt/coffee-ai-boss && docker compose -f deploy/docker-compose.1panel.yml up -d`（重建容器加载新 env） |
| 拉新代码 + 重启 | `cd /opt/coffee-ai-boss && git pull && docker compose -f deploy/docker-compose.1panel.yml up -d --build` |
...
若不想用容器，仍可用 `scripts/start.sh` + `deploy/coffee-ai-boss.service`（见根 `DEPLOY.md`）。1Panel 反代 + SSL 步骤（Step 5/6/7）完全通用，只需把代理地址指向 `http://127.0.0.1:8000`（裸机 uvicorn 监听同样端口）。
