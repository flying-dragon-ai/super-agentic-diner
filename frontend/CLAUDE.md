...
| 时间 | 动作 | 说明 |
|------|------|------|
| 2026-06-28 13:13 | 增量校验（第十次 init） | **3D 社交面板 + Today Topics + 移动端响应式 + 暗色主题 + Dashboard/聊天面板增强**（仅文档，对照 `031608f`→`25ed549`）。前端核心架构（3D 渲染/A\*寻路/坐标投影/事件管线）不变，本次为功能层增量：① **3D 社交面板**（`4031978`，配合后端 `visitor_analytics_service` + `VisitorInsight` 表）；② **Today Topics 面板 + Dashboard/Office Scene/Chat Panel 更新**（`6506e92`）；③ **移动端/平板响应式布局 + drawer 侧栏**（`4b0b736`）；④ **暗色主题**：agent 标签黑色 + 跟随系统偏好自动暗色模式（`821948f`/`850e746`/`13510b1`）；⑤ agent lamps 垂直布局 + 居中 name label（`7d6000d`/`13510b1`）；⑥ 聊天卡片逻辑优化：纯闲聊不出卡片（`a0f69b2`）、从 LLM 回复提取咖啡名作卡片（`b338dee`）。后端数据栈已切 SQLite+fakeredis 默认（见根/app CLAUDE.md），前端无感知变化。详细组件级文档待后续按需补扫 |
| 2026-06-21 | 3D 交互增强（第二/一步） | **碰撞系统调优 + 渲染层验证**（`docs/features/3d-interaction-enhancement-plan.md` 第二步+第一步）。① **碰撞参数调优**（`office3d/core/constants.ts`）：`BUMP_FREEZE_MS` 1500→800、`BUMP_RECOVERY_MS` 1200→600、`AGENT_RADIUS` 20→26、`SEPARATION_STRENGTH` 3→6（更大半径提前避让 + 更短冻结窗口减少"死板"）。② **碰撞恢复增强**（`sim/tick.ts` `makeTick` bumpedUntil 分支）：冻结到期立即 `astar` 重新规划到逃逸点（`applyAgentCollisionBumps` 已设 `targetX/Y`），原先解冻后 `path` 为空要等下一帧 `settleIdle` 重新规划，读感是"冻结→弹跳"，现在连续。③ **第一步渲染层验证**：Playwright 打开 `/3d/scene`，页面 overlay 显示 `WS: open · 在场员工 9`（4 staff + 5 顾客人偶），确认 skill/web 顾客人偶进 3D 场景渲染链路通（事件层 `enter_scene` 广播早已工作）。④ console 的 CORS 错误（`/auth/me`、`/api/office/layout`）是 dev 模式 5174→8000 跨域限制（生产同源无此问题，layout 已 localStorage 降级）。⑤ build 通过（tsc+vite，630 modules，零类型错误）|
...
Crossroads Agent Café 的 3D 可视化前端（**取代** 2D 像素风，与后端 `/ws/visualization` 事件流对接）。**2026-06-20 09:40 起，本模块是项目唯一活跃 UI**（像素 Colyseus 方案与独立 2D 对话页均已移出活跃仓库，归档位置见 `docs/archive-manifest.md`），后端根路由 `/` 已改为直出本前端构建产物。四大职责：
...
- **布局**（全屏 `100vw×100vh`，深色 `#080c12` 背景，monospace 数字）：
  - 顶部：标题 `Crossroads Agent Café · 实时监控大屏` + 当前时间（`zh-CN` 格式）
