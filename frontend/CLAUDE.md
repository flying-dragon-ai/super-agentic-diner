...
| 时间 | 动作 | 说明 |
|------|------|------|
| 2026-06-21 | 3D 交互增强（第二/一步） | **碰撞系统调优 + 渲染层验证**（`docs/3D交互增强-执行计划.md` 第二步+第一步）。① **碰撞参数调优**（`office3d/core/constants.ts`）：`BUMP_FREEZE_MS` 1500→800、`BUMP_RECOVERY_MS` 1200→600、`AGENT_RADIUS` 20→26、`SEPARATION_STRENGTH` 3→6（更大半径提前避让 + 更短冻结窗口减少"死板"）。② **碰撞恢复增强**（`sim/tick.ts` `makeTick` bumpedUntil 分支）：冻结到期立即 `astar` 重新规划到逃逸点（`applyAgentCollisionBumps` 已设 `targetX/Y`），原先解冻后 `path` 为空要等下一帧 `settleIdle` 重新规划，读感是"冻结→弹跳"，现在连续。③ **第一步渲染层验证**：Playwright 打开 `/3d/scene`，页面 overlay 显示 `WS: open · 在场员工 9`（4 staff + 5 顾客人偶），确认 skill/web 顾客人偶进 3D 场景渲染链路通（事件层 `enter_scene` 广播早已工作）。④ console 的 CORS 错误（`/auth/me`、`/api/office/layout`）是 dev 模式 5174→8000 跨域限制（生产同源无此问题，layout 已 localStorage 降级）。⑤ build 通过（tsc+vite，630 modules，零类型错误）|
